...
class InputFrame(Frame):
    def __init__(self, parent, *args, **kwargs):
        ...
        self.allow_numbers = ttk.Checkbutton(self.frame2, text="Insert Random Replacement of Numbers for Letters",
        ...
        self.generate_btn.bind('<Return>', Passphrase.get_passphrase)
...
class Passphrase:
        ...
        if word_list:
            ...
            for word in word_list:
                ...
                if app.input_frame.opt_allow_num.get() != 0:
                    word = self.number_replacement(word)

                if app.input_frame.opt_allow_caps.get() != 0:
                    word = self.capitalise(word)

                # add the processed word to the passphrase, and add a blank space between the words    
                passphrase += (word + " ")
                print(passphrase)

            # guarantee at lease one number in passphrase
            if app.input_frame.opt_allow_num.get() != 0:
                while not self.number_check(passphrase):
                    if not self.substitution_check(passphrase):
                        self.get_passphrase()  # No substitution possible, get new passphrase
                    else:
                        passphrase = self.number_replacement(passphrase)

            # guarantee at lease one number in passphrase
            if app.input_frame.opt_allow_caps.get() != 0:
                while not self.capital_check(passphrase):
                    if not self.substitution_check(passphrase):
                        self.get_passphrase()  # No substitution possible, get new passphrase
                    else:
                        passphrase = self.capitalise(passphrase)

            # display the passphrase, and remove the trailing blank space
            app.output_frame.passphrase_entry.insert(END, (passphrase.rstrip()))

    def number_replacement(self, word):
        modify_word = ""
        for letter in word:
    ...
    def substitution_check(self, passphrase):
        """
        Check if the passphrase contains any letters that can be substituted (e.g., contains only punctuation)
        """
        return any(char in LETTER_TO_NUMBER for char in passphrase)

    def number_check(self, passphrase):
        """
        Check if the passphrase contains any capital letters
        """
        return any(char.isdigit() for char in passphrase)

    def capital_check(self, passphrase):
        """
        Check if the passphrase contains any capital letters
        """
        return any(char.isupper() for char in passphrase)

    def puctuation_check(self, passphrase):
        """
        Check if the passphrase contains any capital letters
        """
        return any(char in string.punctuation for char in passphrase)
